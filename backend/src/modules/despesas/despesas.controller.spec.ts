...
describe('DespesasController', () => {
  ...
  it('findOne delegates with empresaId and id', async () => {
    await controller.findOne('emp-1', 'd-1');
    expect(service.findOne).toHaveBeenCalledWith('emp-1', 'd-1');
  });

  it('create delegates with empresaId and dto', async () => {
    const dto = { descricao: 'Luz' } as any;
  ...
  it('update delegates with empresaId, id and dto', async () => {
    const dto = { descricao: 'Agua' } as any;
    await controller.update('emp-1', 'd-1', dto);
    expect(service.update).toHaveBeenCalledWith('emp-1', 'd-1', dto);
  });

  it('remove delegates with empresaId and id', async () => {
    await controller.remove('emp-1', 'd-1');
    expect(service.remove).toHaveBeenCalledWith('emp-1', 'd-1');
  });
});
